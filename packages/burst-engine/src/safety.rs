...
pub(crate) fn plan_key_up(
    key: KeyId,
    _physical_keys: &PhysicalKeys,
    simulated_keys: &SimulatedKeys,
    _allow_while_physical_down: bool,
) -> Option<KeyEvent> {
    // ⚠️ 不检查物理键状态，无条件发 key_up。
    //
    // 陷阱一（账本）：cancel_all_loops 的 timeout fallback 会 drain 账本后再发 key_up；
    // scheduler 之后处理 StopAll cleanup 若以"账本非空"为前提，账本已空则静默跳过 key_up，
    // 驱动侧按键永久卡住。
    //
    // 陷阱二（物理键）：Toggle 连发激活时用户物理按压 target 键，Stop 命令到达 scheduler
    // 时 physical_keys 含该键；若以"非物理按下"为前提返回 None，模拟 down 永久未释放，
    // 全局开关/切换输入模式均无法恢复，须卸载驱动才能清除。
    // （Windows Fast Startup 下驱动状态随休眠文件恢复，「关机+开机」≠ 冷启动，
    //   只有「重启」才能清除卡住的按键。）
    //
    // 不变式：本函数仅通过 release_target_owner / release_all_target_holds 调用；
    // 两者都只在 target_holds 有 owner 时调用，此时键一定处于 simulated-down 状态，
    // 无需额外条件即可安全发 key_up。账本尽力更新但不作为前置条件。
