...
- 当前基线：`v0.3.1`。已发布 0.2.0–0.3.0；v0.3.1 为本次发布，主题为按键录入策略收敛、配置装载净化与自动更新可控。
- 已完成主能力：按压 / Toggle 连发、键盘全键位、鼠标 5 键、滚轮连发、Toggle 互斥分组（同组互斥切换）、规则拖拽排序与分组折叠/展开、竖版规则列表 + 横版键鼠图双布局、常驻置顶浮窗（显示激活规则 + 全局开关 + 展开主面板）、主题换肤（21 门派配色 + 亮 / 暗 / 跟随系统暗色模式）、多输入模式（主推游戏模式 Interception；通用 SendInput 与 DD驱动 DdSimple 备用，DD-HID 已永久移除；DD驱动与横版互斥）、多配置文件与 `.qzh` 导入导出、全局热键（键盘实体键含左右修饰键、互不重复）、面板显隐热键、设置面板、声音反馈（每项独立开关，可换自选音频）、诊断修复、外部配置导入、自动更新（可开关）、系统托盘、开机自启。
...
std::panic::set_hook → 捕获 panic 信息
  ...
  → 弹崩溃提示窗口（rfd 原生对话框，独立线程展示、进程级去重）
      ┌─────────────────────────────────────┐
      │ 气质花遇到了一个问题并已崩溃          │
      │                                     │
      │ 崩溃日志已保存，如需报告问题请提供：  │
...
弹窗在独立线程展示（panic 可能发生在 LL hook 等敏感线程，原地阻塞会拖垮系统输入）；
正文直接展示崩溃日志完整路径，故不单设「复制路径」按钮。

#### 用户反馈引导

- 关于 / 诊断入口提供"打开日志目录"入口（日常排查用）
- 崩溃弹窗提供"打开日志文件夹"按钮，正文展示崩溃日志完整路径
...
进度：面板窗口、托盘、更新检查、驱动诊断已落地；桌宠窗口、许可证激活分别进入 v0.4 / v0.5；产品页与发布镜像已上线，更新器镜像端点待随应用发版（v0.6）。
...
- [x] 托盘图标 + 菜单（菜单文字与图标均区分启用/禁用状态）
- [x] 托盘菜单：全局开关 / 打开面板 / 退出

**自动更新**

- [x] `tauri-plugin-updater` 集成（插件注册完成）
- [x] 「自动更新」开关（设置 → 通用，默认开启，存 settings.json 的 `autoUpdate`，前后端同读）
- [x] 开启时：启动静默检查并下载，下载完成弹更新公告弹窗（含 Release 正文），可「稍后处理」或「重启并更新」
- [x] 关闭时：只发 `update-available`，标题栏亮可用更新提示，点击后走同一下载流程
- [x] 用户手动"检查更新"一律下载（已表达更新意图），无更新时提示"已是最新版本"
- [x] 菜单 / 关于里的「更新公告」固定展示**当前运行版本**那一节（随包内联的 `CHANGELOG.md`），与新版本公告分开
...
- [x] 崩溃提示窗口（打开日志文件夹 / 关闭；正文展示崩溃日志路径）
- [x] 托盘启用 / 禁用动态图标切换（禁用态灰化图标）
...
- [x] 导入 / 导出原生 `.qzh` 文件（跨设备迁移、社区分享）
- [ ] `notify` 监听配置文件变更自动 reload；失效时定时轮询兜底
- [x] 托盘菜单：切换配置文件（动态菜单项）

**交互优化**

- [x] 连发间隔步进器（数值输入外增加 ±10ms 快速调节，竖版规则卡片与横版统一间隔共用）
- [x] 提示音支持自选音频：四个播报时机各自在「朗读」与「音频」间切换，音频文件导入时复制进 `app_data_dir/sounds`
- [x] 用户协议与更新公告改用自研轻量 Markdown 渲染（`components/markdown-parse.ts` + `Markdown.tsx`，不产 `<a>`、不用 `dangerouslySetInnerHTML`）

**按键录入与配置装载收敛（0.3.1）**

- [x] 按键角色模型落到 `qzh-profile/src/key_policy.rs`：读 / 写角色 + 五个 `KeySlot`，重合槽位取交集并按规则模式再分一层
- [x] `get_key_policy` 按当前输入模式下发允许集，前端不再自维护白名单，不支持的键在录入时就被拒并说明原因
- [x] 全局热键放开左右修饰键（Shift / Ctrl / Alt / Win），连发规则仍禁用；绑定右 Alt 后按是否也绑左 Ctrl 分两级提示 AltGr 坑
- [x] 所有配置装载统一经 `activate_profile_file` → `sanitize_profile`：启动、托盘切换、`load_profile`、导入 `.qzh`、外部导入、删除后回落共六条路径
- [x] 永久移除 DD-HID：注入后端、`InputMode::DdHid`、安装链路与用户态 DLL 全部删除，只保留卸载与残留清理
- [x] 用户协议升级至 v1.4：2.3 节由「DDHID 暂停使用」改写为「已移除」，`AGREEMENT_VERSION` 同步升到 1.4（老用户升级后需重新同意一次）
- [x] 自动更新开关（默认开启）：关闭时只在标题栏提示可用更新，菜单「检查更新」一律下载；更新公告区分当前版本与新版本
...
- [x] 发版护栏写入 `CLAUDE.md` 发版流程：高风险版本**禁止裸 bump `CURRENT_SCHEMA_VERSION`**（新字段一律 `#[serde(default)]` 走兼容；否则回退到旧 schema 会 `TooNew` 拒载、砸用户配置）；风险改动尽量挂运行时开关，先关开关而非全量回退
- [x] 保留上个稳定版安装包 + `.sig`（旧 GitHub Release 不删除），确保向前滚 / 手动回退有可用且已签名的产物
...
- [x] 服务器开通与首次上线：`site-v0.1.0` 已发，https://app.xwink.fun/flair-bloom/ 可下载，镜像定时器每 10 分钟同步
