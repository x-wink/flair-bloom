...
连发优化以"物理输入优先、异常 fail-closed、功能语义不破坏"为硬约束，详见 `docs/BURST_ENGINE_OPTIMIZATION.md`。

- **不吞物理输入**：低级 hook 只观察和转发输入，所有路径必须继续传递给系统；任何优化不得把物理键盘 / 鼠标事件作为可拦截资源处理。
- **同键 Hold 是核心语义**：`Hold trigger_key == target_key` 是按压连发的正常用法，表示用户按住触发键时输出周期性 `down/up` 脉冲；不得作为异常配置禁用。
- **正常操作优先**：异键规则或 Toggle 规则若与用户真实按住的目标键冲突，优先保护用户真实输入；对 WASD / 方向键 / Space / Shift / Ctrl / 鼠标拖拽键等高风险目标给出明确提示。
...
| 目标               | 要求                                                                 |
| ------------------ | -------------------------------------------------------------------- |
| 物理输入连续性     | 用户真实按键 / 鼠标事件始终传给系统，正常 WASD 移动不被引擎主动拦截 |
| 同键 Hold 可用性   | `trigger_key == target_key` 的按压连发保持可用，松开触发键立即停止   |
