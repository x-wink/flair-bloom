...
- 当前基线：`v0.2.8`，已发布 0.2.0–0.2.8，主题为体验完善、稳定性与完整配置管理。
- 已完成主能力：按压 / Toggle 连发、键盘全键位、鼠标 5 键、滚轮连发、Toggle 互斥分组（同组互斥切换）、规则拖拽排序与分组折叠/展开、多输入模式（主推游戏模式 Interception；通用 SendInput 与 DD驱动 DdSimple 备用；DDHID 已禁用）、多配置文件、全局热键（仅键盘、互不重复）、面板显隐热键、设置面板、声音反馈、诊断修复、外部配置导入、自动更新、系统托盘、开机自启。
