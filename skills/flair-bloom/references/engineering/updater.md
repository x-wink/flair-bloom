...
gh-proxy 是第三方免费服务，它挂了就退到 GitHub 直连（国内约 20 KB/s，基本等于不可用）。这不是撤镜像换来的新风险：原先的服务器镜像同步本身也走 gh-proxy、失败才回落同样的直连，代理挂掉时镜像照样拿不到新版本，单点依赖始终是这一个。撤掉换来的是零带宽成本与零同步基础设施；失去的只有「已经同步落盘的旧版本在代理不可用时仍能下载」这一点。镜像那套要回来按 git 历史恢复。
