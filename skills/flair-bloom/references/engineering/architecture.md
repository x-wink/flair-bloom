...
- 新手引导 `tour/`：`TourRunner.tsx` 是唯一有 DOM 逻辑的文件（四块遮罩镂空、锚定气泡、实操判定），教程内容在 `tour/tours/<id>.ts`，只经 `TourHost` 接口操作宿主；进度存 `settings.json` 的 `tours` 键。说明书大纲在本 skill 的 `references/manual/<id>.md`，`pnpm skills:check` 钉住一一对应。
